...
export function QuizModal({ isOpen, onClose, initialNote }: QuizModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        setIsSuccess(true);
        // @ts-expect-error: Yandex Metrika global injected by script
        if (typeof ym !== "undefined") {
          // @ts-expect-error: Yandex Metrika global injected by script
          ym(100155250, "reachGoal", "quiz_submitted");
        }
      } else {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <FocusTrap>
        ...
        >
          <h2 id="quiz-title" className="sr-only">Рассчитать стоимость проекта</h2>
        {/* Close button */}
        <button
          onClick={reset}
          ...
          <>
            ...
            <div className="bg-primary p-8">
              ...
              <div className="flex items-center gap-2 mt-6">
                {STEPS.map((step) => (
                  <div key={step.id} className="flex-1 h-2 rounded-full bg-white/20 overflow-hidden">
                    <div
                      className={cn(
                ...
                <div className="space-y-6">
                  ...
                  {selectedServices.length > 0 && (
                    <p className="text-sm text-muted-foreground text-center">
                      Выбрано систем: <span className="font-bold text-foreground">{selectedServices.length}</span>
                    </p>
                  )}
                </div>
              )}
              ...
              {currentStep === 4 && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label htmlFor="quiz-name" className="block text-xs font-black uppercase tracking-widest text-muted-foreground mb-2">
                        Имя *
                      </label>
                      <input
                        id="quiz-name"
                    ...
                    <div>
                      <label htmlFor="quiz-phone" className="block text-xs font-black uppercase tracking-widest text-muted-foreground mb-2">
                        Телефон *
                      </label>
                      <input
                        id="quiz-phone"
                    ...
                    <div>
                      <label htmlFor="quiz-note" className="block text-xs font-black uppercase tracking-widest text-muted-foreground mb-2">
                        Комментарий
                      </label>
                      <textarea
                        id="quiz-note"
