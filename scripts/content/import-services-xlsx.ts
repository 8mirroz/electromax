...
export const SolutionKitSchema = z
  .object({
...
async function main() {
  ...
  const filePath = path.resolve(
    process.cwd(),
    options.input || path.join("scripts", "content", "services-master.xlsx"),
  );
  const outputDir = path.resolve(process.cwd(), options.output || path.join("src", "generated"));

  if (!fs.existsSync(filePath)) {
    console.warn(
      `[ContentOps] Master XLSX file not found at ${filePath}. Creating a strict basic template for Agent 1...`,
    );
  ...
  const titleMap = generatedPages
    .map((service) => `  ${JSON.stringify(service.slug)}: ${JSON.stringify(service.title)},`)
    .join("\n");
  const shortNameMap = generatedPages
    .map((service) => `  ${JSON.stringify(service.slug)}: ${JSON.stringify(service.shortName)},`)
    .join("\n");
